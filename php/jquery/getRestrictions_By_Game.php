...
if(security_check($_SESSION['safe_key'],$_SESSION['user_id'])==true)
{
	...
	$sql="Select R.id ,U.name ,get_number_of_restrictions(U.id) as n_o_r,U.surname,R.time_to,R.time_from,R.date from user U,restriction R where U.id=R.user_id AND R.time_to>:game_start_time AND R.time_from < :game_end_time AND R.date=:date order by n_o_r desc limit :page,3";
	$run = $dbh->prepare($sql);
