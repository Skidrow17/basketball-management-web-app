...
if(isset($_SESSION['safe_key']) && isset($_SESSION['user_id']) && isset($_SESSION['profession'])){
    if (security_check($_SESSION['safe_key'], $_SESSION['user_id']) == true && $_SESSION['profession'] == 'Admin') {
        if ($_GET['id'] == 1) {
            $sql = "SELECT U.name,U.surname,R.time_from,R.time_to,R.date,R.register_timestamp,R.deletable,R.comment 
                    FROM restriction R, user U 
                    WHERE u.Id = R.user_id AND register_timestamp > :date_from";
            ...
            $sql = "SELECT u.name AS user_name,u.surname,home.name as home_team,away.name as away_team,g.date_time,team_score_1,team_score_2,c.name AS court_name 
                    FROM user u,human_power hp,court c,game g JOIN team AS home ON g.team_id_1 = home.id JOIN team AS away ON g.team_id_2 = away.id 
                    WHERE hp.game_id = g.id and hp.user_id = u.id and g.court_id = c.id and g.date_time > :date_from  
