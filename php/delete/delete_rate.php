...
if (isset($_POST['rate_id']) && isset($_SESSION['safe_key']) && isset($_SESSION['user_id'])) {
    if (security_check($_SESSION['safe_key'], $_SESSION['user_id']) == true && $_SESSION['profession'] === 'Admin') {
        $id = filter_var($_POST['rate_id'], FILTER_SANITIZE_NUMBER_INT);
        $sql = "UPDATE rate SET active=1 where id = :id";
        $run = $dbh->prepare($sql);
