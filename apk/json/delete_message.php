...
if (security_check($_GET['safe_key'], $_GET['user_id']) == true) {
	if ($_GET['type'] !== 'incomming') {
		$sql = "UPDATE message SET sender_delete=1 WHERE id=:id AND sender_id=:sender_id";
		$run = $dbh->prepare($sql);
		...
		$sql = "UPDATE message SET receiver_delete=1 WHERE id=:id AND receiver_id=:receiver_id";
		$run = $dbh->prepare($sql);
		$run->bindValue(':id', $_GET["message_id"]);
