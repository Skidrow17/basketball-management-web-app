...
if (security_check($_GET['safety_key'], $_GET['id']) == true) {
	...
	$sql3 = "UPDATE login_history SET logout_date_time=? WHERE user_id=? order by id desc Limit 1";
	...
	$sql = "Select A.title,A.text,U.name,U.surname from announcement A,user U where U.id=A.user_id order by date_time desc Limit 1";
	$run = $dbh->prepare($sql);
	$run->execute();
