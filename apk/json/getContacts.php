...
if(isset($_GET['safe_key']) && isset($_GET['id'])){
	...
	if (security_check($_GET['safe_key'], $_GET['id']) == true) {
		...
		$sql = "SELECT get_last_login_by_user(U.id) as last_login,UC.name as profession,U.id,U.name,U.surname,U.profile_pic,U.phone from user U,user_categories UC where U.id!=:id AND UC.id = U.profession AND U.active = 0 order by name asc";
		$run = $dbh->prepare($sql);
